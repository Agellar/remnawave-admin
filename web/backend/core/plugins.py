...
def discover_plugins() -> list[PluginManifest]:
    ...
    entry_factories = list(_iter_entry_point_factories())
    dev_factories = list(_iter_dev_factories())
    prefer_dev = os.environ.get("RWA_DEV_PLUGINS_PREFER", "").strip().lower() in {
        "1", "true", "yes", "on",
    }
    factories = (
        dev_factories + entry_factories
        if prefer_dev
        else entry_factories + dev_factories
    )
