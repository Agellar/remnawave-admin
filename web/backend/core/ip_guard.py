...
def _tone(group: Dict[str, Any]) -> str:
    """Насколько сигнал весомый: первый порог — повод проверить, не обвинение."""
    if not group.get("metadata_known") or group.get("is_mobile"):
        return "warning"
    return "critical" if group.get("accounts", 0) >= 6 else "warning"
...
async def run_once() -> int:
    ...
    threshold = _int_setting("violations_ip_trial_accounts", 3)
    mobile_threshold = _int_setting("violations_ip_trial_accounts_mobile", 0)
    window_days = _int_setting("violations_ip_trial_window_days", 14)
    repeat_hours = _int_setting("violations_ip_trial_repeat_hours", 168)
    if threshold <= 0:
        return 0

    # Settings express the allowed count, not the first bad count. Query from
    # the next account so the UI wording and detector behaviour agree.
    query_thresholds = [threshold + 1]
    if mobile_threshold > 0:
        query_thresholds.append(mobile_threshold + 1)
    groups = await db_service.get_shared_ip_accounts(
        min_accounts=min(query_thresholds), days=window_days, limit=50,
    )
    if not groups:
        return 0
    ...
    for group in groups:
        ...
        # Без GeoIP нельзя отличить домашний адрес от мобильного CGNAT. А
        # публичный VPN/хостинг объединяет заведомо разных людей. В обоих
        # случаях совпадение адреса не является пригодным abuse-сигналом.
        if not group.get("metadata_known"):
            continue
        if group.get("is_hosting") or group.get("is_proxy"):
            continue
        if group.get("is_mobile") and mobile_threshold <= 0:
            continue
