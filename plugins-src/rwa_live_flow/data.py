...
async def _profiles_cached(logger) -> tuple[dict[str, dict] | None, bool]:
    ...
    # Несколько вкладок могут попасть ровно на истечение TTL. Только одна из
    # них обновляет профили; остальные получают уже обновлённый кэш.
...
# Detail endpoints page by *users*, not raw IP rows.  A user can have many
# recent IPv4/IPv6 connections, so both dimensions need a hard cap before the
# response reaches JSON encoding and the browser DOM.
DETAIL_DEFAULT_PAGE_SIZE = 75
DETAIL_MAX_PAGE_SIZE = 150
DETAIL_MAX_PAGE = 200
MAX_IPS_PER_USER = 6
...
async def _active_by_node(ctx, scope) -> dict[str, int] | None:
    ...
    try:
        rows = await ctx.db.fetch(
            """
            WITH parsed AS (
                SELECT raw_data::jsonb->'userTraffic'->>'lastConnectedNodeUuid' AS nu,
                       CASE WHEN raw_data::jsonb->'userTraffic'->>'onlineAt' ~
                           '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\\.[0-9]+)?(Z|[+-][0-9]{2}:?[0-9]{2})?$'
                           THEN (raw_data::jsonb->'userTraffic'->>'onlineAt')::timestamptz
                       END AS online_at
                FROM users
                WHERE ($2::uuid[] IS NULL OR uuid = ANY($2::uuid[]))
                  AND ($3::text[] IS NULL OR raw_data::jsonb->'userTraffic'->>'lastConnectedNodeUuid' = ANY($3::text[]))
            ),
            active AS (
                SELECT * FROM parsed
...
def _active_counts(users: list[tuple[str, dict]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for _uid, user in users:
        node_uuid = user.get("node_uuid")
        if node_uuid:
            counts[str(node_uuid)] = counts.get(str(node_uuid), 0) + 1
    return counts


def _active_ref_iso(users: list[tuple[str, dict]]) -> str | None:
    timestamps = [user.get("online_at") for _uid, user in users if user.get("online_at")]
    ref = max(timestamps) if timestamps else None
    return ref.astimezone(timezone.utc).isoformat() if ref else None


async def collect(ctx, scope=None) -> dict:
    ...
    """
    from .poller import MAX_USERS, POLLER, USERS_PAGE
    ...
    if live:
        visible_active = scope.filter_active(POLLER.active_users())
        active = _active_counts(visible_active)
        active_as_of = _active_ref_iso(visible_active)
        split, total_split = await _vpn_split(ctx, POLLER, visible_active)
    else:
        act = await _active_by_node(ctx, scope)
    ...
    rows = await ctx.db.fetch(
        ...
        WHERE is_disabled = false
          AND ($1::uuid[] IS NULL OR uuid = ANY($1::uuid[]))
        ...
        nodes.append(
            {
                ...
                "users": (
                    (int(active.get(row["uuid"], 0)) if active is not None else 0)
                    if scope.users_restricted
                    else int((POLLER.nodes.get(row["uuid"]) or {}).get("users_online", row["users_online"] or 0))
                    if live
                    else int(row["users_online"] or 0)
                ),
    ...
    return {
        ...
        "poll_user_limit": MAX_USERS,
        "poll_page_size": USERS_PAGE,
        "nodes": nodes,
        "sinks": ordered,
...
async def node_users(
    ...
    *,
    page: int = 1,
    limit: int = DETAIL_DEFAULT_PAGE_SIZE,
) -> dict | None:
    ...
    отсчитывается от среза; дата кастуется только после regex-проверки). В
    ...
    scope = scope or AccessScope.unrestricted()
    node_uuid = str(node_uuid).lower()
    # Avoid even probing the nodes table for an object outside the policy.
    # The route maps this to the same 404 used for a nonexistent node.
    if not scope.allows_node(node_uuid):
        return None
    page = max(1, min(int(page), DETAIL_MAX_PAGE))
    limit = max(1, min(int(limit), DETAIL_MAX_PAGE_SIZE))
    ...
    node = await ctx.db.fetchrow(
        ...
        "AND ($2::uuid[] IS NULL OR uuid = ANY($2::uuid[]))",
        node_uuid,
        scope.node_sql_arg(),
    )
    if not node:
        return None
    if POLLER.fresh:
        return await _node_users_live(ctx, node, node_uuid, POLLER, scope, page=page, limit=limit)
    try:
        return await _node_users_db(ctx, node, node_uuid, scope, page=page, limit=limit)
        ...
        return {
            "node": {
                "uuid": node["uuid"],
                "name": node["name"],
                "users_online": 0 if scope.users_restricted else int(node["users_online"] or 0),
                "counter_scoped": scope.users_restricted,
            },
            "users": [], "count": 0, "window_s": int(ONLINE_WINDOW_S + SYNC_LAG_S),
            "as_of": None, "source": "db-sync", "unavailable": True,
            "page": page, "page_size": limit, "has_more": False, "truncated": False,
        }


async def _node_users_db(ctx, node, node_uuid: str, scope, *, page: int, limit: int) -> dict:
    offset = (page - 1) * limit
    rows = await ctx.db.fetch(
        """
        WITH parsed AS (
            SELECT u.uuid AS user_uuid, u.username, u.email, u.telegram_id, u.tag,
                   CASE WHEN u.raw_data::jsonb->'userTraffic'->>'onlineAt' ~
                       '^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}(\\.[0-9]+)?(Z|[+-][0-9]{2}:?[0-9]{2})?$'
                       THEN (u.raw_data::jsonb->'userTraffic'->>'onlineAt')::timestamptz
                   END AS online_at
            FROM users u
            WHERE u.raw_data::jsonb->'userTraffic'->>'lastConnectedNodeUuid' = $1
              AND ($3::uuid[] IS NULL OR u.uuid = ANY($3::uuid[]))
        ),
        online AS (
            SELECT * FROM parsed
        ...
        page_users AS (
            SELECT * FROM online
            ORDER BY online_at DESC, user_uuid
            LIMIT $4 OFFSET $5
        )
        SELECT t.total, t.as_of, o.user_uuid::text AS user_uuid,
               o.username, o.email, o.telegram_id, o.tag, o.online_at,
        ...
        FROM totals t
        LEFT JOIN page_users o ON true
        LEFT JOIN LATERAL (
            SELECT dedup.ip_address, dedup.connected_at, dedup.inbound
            FROM (
                SELECT DISTINCT ON (c.ip_address)
                       c.ip_address, c.connected_at,
            ...
            LIMIT $6
        ) i ON o.user_uuid IS NOT NULL
        LEFT JOIN ip_metadata m ON m.ip_address = i.ip_address
        ORDER BY o.online_at DESC NULLS LAST, o.user_uuid, i.connected_at DESC NULLS LAST
        """,
        node_uuid,
        ONLINE_WINDOW_S + SYNC_LAG_S,
        scope.user_sql_arg(),
    ...
    return {
        "node": {
            "uuid": node["uuid"],
            "name": node["name"],
            "users_online": total if scope.users_restricted else int(node["users_online"] or 0),
            "counter_scoped": scope.users_restricted,
        },
        "users": users,
        "count": total,
...
async def _classify_users(ctx, ids_str: list) -> dict[str, str]:
    ...
    async with _cls_lock:
        # A concurrent /data request may have filled the same IDs while this
        # coroutine waited.  Re-evaluate inside the lock before querying.
        now = _time.time()
        cached = _cls_cache["map"] if now - _cls_cache["ts"] < CLASSIFY_TTL_S else {}
        missing = [u for u in wanted if u not in cached]
        if missing:
            ...
            cached = merged
        return {u: cached.get(u, "unknown") for u in wanted}
...
async def _vpn_split(ctx, poller, live_users: list | None = None) -> tuple[dict[str, dict], dict]:
    ...
    """
    act = poller.active_users() if live_users is None else live_users
    if not act:
        ...
        ip_rows = await ctx.db.fetch(
            ...
            ranked AS (
                SELECT *, row_number() OVER (
                    ...
                    ORDER BY connected_at DESC NULLS LAST, ip_address
                ) AS rn
                FROM distinct_ips
            )
            SELECT c.user_uuid::text AS user_uuid, c.ip_address::text AS ip_address, c.connected_at,
                   c.inbound,
                   m.asn, m.asn_org, m.country_code, m.city,
                   m.is_mobile, m.is_hosting, m.is_vpn, m.is_proxy
            FROM ranked c
            LEFT JOIN ip_metadata m ON m.ip_address = c.ip_address
            WHERE c.rn <= $2
            ORDER BY c.user_uuid, c.connected_at DESC NULLS LAST, c.ip_address
            """,
            uuids,
            MAX_IPS_PER_USER,
        )
...
def _sorted_live_users(live_users: list[tuple[str, dict]]) -> list[tuple[str, dict]]:
    """Stable newest-first ordering used before pagination."""
    floor = datetime.min.replace(tzinfo=timezone.utc)
    return sorted(
        live_users,
        key=lambda item: (item[1].get("online_at") or floor, str(item[0])),
        reverse=True,
    )


async def _node_users_live(ctx, node, node_uuid: str, poller, scope, *, page: int, limit: int) -> dict:
    """То же, что node_users, но онлайн — из живого опроса панели (без лага синка)."""
    live_users = [
        (uid, user)
        for uid, user in scope.filter_active(poller.active_users())
        if user.get("node_uuid") == node_uuid
    ]
    live_users = _sorted_live_users(live_users)
    total = len(live_users)
    offset = (page - 1) * limit
    page_users = live_users[offset:offset + limit]
    users = await _users_rows(ctx, poller, page_users)
    live_node = poller.nodes.get(node_uuid) or {}
    return {
        "node": {
            "uuid": node["uuid"],
            "name": node["name"],
            "users_online": total if scope.users_restricted else int(live_node.get("users_online", node["users_online"] or 0)),
            "counter_scoped": scope.users_restricted,
        },
        "users": users,
        "count": total,
...
async def group_users(
    ...
    *,
    ...
    if not POLLER.fresh:
        return {
            "group": group, "users": [], "count": 0,
            "window_s": int(ONLINE_WINDOW_S), "as_of": None,
            "source": "db-sync", "unavailable": True,
            "page": page, "page_size": limit, "has_more": False, "truncated": False,
        }
    act = scope.filter_active(POLLER.active_users())
    if group == "all":
        chosen = act
    else:
        cls = await _classify_users(ctx, [uid for uid, _ in act])
        chosen = [(uid, u) for uid, u in act if cls.get(str(uid), "unknown") == group]
    chosen = _sorted_live_users(chosen)
    total = len(chosen)
    offset = (page - 1) * limit
    users = await _users_rows(ctx, POLLER, chosen[offset:offset + limit], with_node=True)
    mbps = 0.0
    for uid, _ in chosen:
