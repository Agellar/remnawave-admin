...
UPSTREAM_COMMIT = "a0962f623afcacd4c7cd96b472ec8eae76885e76"
FORK_VERSION = "0.16.0+agellar.1"


def _own_version() -> str:
    """Return fork metadata only when an installed fork wheel matches it.

    Production bakes this source into an immutable backend image. An unrelated
    upstream wheel must not make this hardened fork advertise itself as the
    unpatched public build.
    """
    from importlib.metadata import PackageNotFoundError, version

    try:
        installed = version(DIST_NAME)
        return installed if installed == FORK_VERSION else FORK_VERSION
