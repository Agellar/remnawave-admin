...
def _bounded_env_int(name: str, default: int, minimum: int, maximum: int) -> int:
    """Read a deployment knob without allowing an unbounded poll workload."""
    try:
...
CLOCK_SKEW_S = 30.0
USERS_PAGE = _bounded_env_int("RWA_LIVE_FLOW_USERS_PAGE", 500, 250, 500)
MAX_USERS = max(
    USERS_PAGE,
    _bounded_env_int("RWA_LIVE_FLOW_MAX_USERS", 5000, 500, 20000),
)  # дальше не листаем; срез помечается truncated
...
class PanelPoller:
    ...
    async def _tick_impl(self, log: Any) -> None:
        ...
        total: int | None = None
        while True:
            request_size = min(USERS_PAGE, MAX_USERS - start)
            raw_chunk, total = _response_list(
                await self._fetch(api.get_users(start=start, size=request_size, skip_cache=True)),
                "users",
            )
            # A malformed/misbehaving API must not bypass the requested page
            # bound or make pagination jump by unrequested elements.
            if len(raw_chunk) > request_size:
                log.warning(
                    "live_flow: panel returned %d users for requested page size %d; extras ignored",
                    len(raw_chunk),
                    request_size,
                )
            chunk = raw_chunk[:request_size]
