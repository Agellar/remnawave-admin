...
(function () {
  ...
  function ensureStyle() {
    ...
    st.textContent =
      ...
      V + '.lf-pager{display:flex;align-items:center;justify-content:flex-end;gap:8px;padding:8px 0 2px;color:hsl(var(--muted-foreground, 220 9% 56%));font-size:12px}' +
      V + '.lf-pager button{background:none;border:1px solid hsl(var(--border, 220 14% 18%));color:inherit;border-radius:6px;padding:3px 9px;font:inherit;cursor:pointer}' +
      V + '.lf-pager button:hover:not(:disabled){border-color:hsl(var(--primary, 239 84% 67%) / .6);color:hsl(var(--foreground, 220 9% 84%))}' +
      V + '.lf-pager button:disabled{opacity:.4;cursor:default}' +
  ...
  function paintPanel(view, pd) {
    ...
    if (currentPage > 1 || pd.has_more) {
      h += '<div class="lf-pager"><button type="button" class="lf-prev"' + (currentPage <= 1 ? ' disabled' : '') + '>← ' + tt.pPrev + '</button>'
  ...
  function unmount() {
    ...
    selected = null; panelData = null; panelPage = 1;
