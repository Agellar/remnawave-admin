...
async def handle_ip_action(callback: CallbackQuery, admin: BotAdmin) -> None:
    ...
    required_permission = {
        "block": ("violations", "resolve"),
        "mute": ("violations", "resolve"),
        "users": ("users", "view"),
    }.get(action)
    if required_permission and not await admin.has_permission(*required_permission):
    ...
    if action == "block":
        ...
        await _show_users(callback, ip, admin)
    elif action == "mute":
...
async def _show_users(callback: CallbackQuery, ip: str, admin: BotAdmin) -> None:
    ...
    if visible is not None:
        users = [u for u in users if str(u.get("uuid") or "") in visible]
    if not users:
