...
async def _block_radar(callback: CallbackQuery, action: str, ref: str) -> None:
    ...
    # Official wheels and our self-hosted implementation deliberately share
    # the plugin id/UI, but keep separate tables. Both queries are static: no
    # identifier comes from callback_data.
    queries = (
    ...
    async with db_service.acquire() as conn:
        for query in queries:
            try:
                value = await conn.fetchval(query, alert_id, verdict)
                schema_found = True
            except (UndefinedColumnError, UndefinedTableError) as exc:
                last_schema_error = exc
                continue
            if value is not None:
                updated = value
                break
    if not schema_found and last_schema_error is not None:
