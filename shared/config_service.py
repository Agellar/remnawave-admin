...
DEFAULT_CONFIG_DEFINITIONS: List[Dict[str, Any]] = [
    ...
    {
        "key": "violations_geo_max_simultaneous_countries",
        "value_type": "int",
        "category": "violations",
        "subcategory": "thresholds",
        "display_name": "Макс. одновременных стран",
        "description": "Сколько стран может быть активно одновременно без нарушения. Геолокация IP неточна, а одна подписка часто используется семьёй и на нескольких устройствах. 0 = автоматически по числу увиденных устройств",
        "default_value": "3",
        "sort_order": 12,
    },
    {
        "key": "violations_profile_unusual_hour_enabled",
        "value_type": "bool",
        "category": "violations",
        "subcategory": "thresholds",
        "display_name": "Нетипичное время подключения",
        "description": "Учитывать подключение вне обычных часов как слабый сигнал. По умолчанию выключено из-за частых ложных срабатываний",
        "default_value": "false",
        "sort_order": 13,
    },
    {
    ...
    },
    {
        ...
        "description": "Замечать адреса, с которых пробную подписку брали несколько раз разными аккаунтами. HWID называет сам клиент и подделывается заголовком, а адрес выдаёт провайдер — сменить его труднее. Приватные, мобильные, hosting/proxy и ещё не классифицированные адреса по умолчанию не учитываются",
        "default_value": "true",
        "sort_order": 26,
    },
    {
        ...
        "description": "Сколько разных пробных подписок с одного адреса допустимо без сигнала; срабатывание начинается со следующей. Считаются только пробные: за адресом домашнего провайдера живёт целая квартира. 0 = отключить сторож полностью",
        "default_value": "3",
        "sort_order": 27,
    },
    {
        ...
        "description": "Отдельный допустимый максимум для мобильных операторов. Там CGNAT: за одним адресом сидит район, поэтому мобильные адреса по умолчанию не оцениваются. 0 = не проверять мобильные адреса",
        "default_value": "0",
        "sort_order": 28,
    },
    {
